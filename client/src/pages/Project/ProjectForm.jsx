...
export default function ProjectForm({ initial, onSubmit, onCancel }) {
  ...
  const submit = (e) => { e.preventDefault(); onSubmit({ name, description, status, budget: Number(budget||0), startDate, endDate, office }) }
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        ...
        <label className="space-y-1"><span className="text-sm">Status</span>
        ...
        <label className="space-y-1 col-span-2"><span className="text-sm">Description</span><textarea className="w-full border rounded-xl px-3 py-2" value={description} onChange={e=>setDescription(e.target.value)} /></label>
        <label className="space-y-1"><span className="text-sm">Budget</span><input className="w-full border rounded-xl px-3 py-2" type="number" value={budget} onChange={e=>setBudget(e.target.value)} /></label>
        ...
        <label className="space-y-1"><span className="text-sm">Start</span><input className="w-full border rounded-xl px-3 py-2" type="date" value={startDate} onChange={e=>setStartDate(e.target.value)} required /></label>
        <label className="space-y-1"><span className="text-sm">End</span><input className="w-full border rounded-xl px-3 py-2" type="date" value={endDate} onChange={e=>setEndDate(e.target.value)} /></label>
      </div>
